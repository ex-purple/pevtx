...
void binxml_parser::parse(std::istream &stream, chunk &chunk, binxml_node &node, bool template_definition)
{
    set_stream(stream);
    current_chunk = &chunk;
    root = &node;
    ...
    {
	auto sym = get();
//std::cout << "token: " << std::showbase << std::hex << sym << std::dec << std::endl;
